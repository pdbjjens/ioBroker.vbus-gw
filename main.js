...
const { Connection, TcpConnectionEndpoint } = vbus;
...
class VbusGw extends utils.Adapter {
    ...
    async createTcpEndpoint() {
        ...
        endpoint.on('connection', connectionInfo => {
            ...
            if (serialPort) {
                this.log.info(
                    `Negotiated connection with ${connectionInfo.socket.remoteAddress.replace(/^.*:/, '')} for channel ${channel}...`,
                );
        ...
        endpoint.on('connectionAttemptFailed', connection => {
            if (connection.error) {
                this.log.info(
                    `Rejected connection with ${connection.ip.replace(/^.*:/, '')} due to ${connection.error}...`,
                );
            }
        });
        ...
        port.on('readable', () => {
            ...
            while ((chunk = port.read())) {
                for (const connection of connections) {
                    if (connection.channel === config.channel) {
